...
除 filter 外，**dangkou / peijian / pizhi / datu 都需要一个配置表**：
...
**禁止**：
- 不读 `*_config.json`（CLI 不依赖，必须显式传配置表路径）
- 不用缓存的旧路径（即使文件还在那）
...
```
如需修改，直接编辑 binary 同目录下的 `keywords.json`，或用 GUI 齿轮按钮。
...
> **🆕 v2026-07-01 binary 升级**：CLI 必须显式传配置文件（`<自设编码.xlsx>` 等）。
...
### filter 关键词（如需自定义）
filter 命令读取 keywords.json（默认随 binary 一起发布）：
```json
{
  "doubtKeywords": ["其他", "咨询客服", "备注", "diy"],
  "accessoryKeywords": ["支架", "绳", "链", "吸盘", "串珠", "相机", "纽扣", "腕带", "贴纸", "卡包"]
...
3. **没有默认动作**（2026-07-01 老板指定）：老板必须明确点名跑哪个命令才执行；说「处理订单」我只是列菜单
4. **配件编码校验**：SKU 中的配件个数必须等于编码列数，错了会直接报错
